...
struct Command commands[] =
{
		...
		{ "schedPRIRR", "switch the scheduler to RR with given #priorities & quantum and starvation threshold" ,command_sched_init_PRIRR, 3},
...
int command_sched_init_PRIRR(int number_of_arguments, char **arguments)
{
	...
	cprintf("Scheduler is now set to Priority Round Robin with:\n");
	cprintf("  Number of Priorities: %d\n", numOfPriorities);
	cprintf("  Quantum: %d ms\n", quantum);
	cprintf("  Starvation Threshold: %u ms\n", starvThresh);
